...
from sqlalchemy.dialects.postgresql import JSONB as PostgresJSONB, ARRAY as PostgresARRAY
...
class ChannelAccount(Base):
    ...
    id = Column(SafeUUID, primary_key=True, default=uuid.uuid4)
    client_id = Column(SafeUUID, ForeignKey("clients.id", ondelete="CASCADE"), nullable=False)
    channel = Column(String(40), nullable=False)  # Enums treated as String here
...
class Contact(Base):
    ...
    source_channel = Column(String(40))
...
class LeadProfile(Base):
    ...
    stage = Column(String(40), nullable=False, default="new_lead")
...
class Conversation(Base):
    ...
    channel_account_id = Column(SafeUUID, ForeignKey("channel_accounts.id"), nullable=True)
    channel = Column(String(40), nullable=False)
    status = Column(String(40), nullable=False, default="bot_active")
...
class Passenger(Base):
    ...
    status = Column(String(40), nullable=False, default="placeholder")
    meta_data = Column("metadata", SafeJSONB, nullable=False, default={})
    created_at = Column(DateTime(timezone=True), nullable=False, default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, default=func.now(), onupdate=func.now())
...
class Invoice(Base):
    ...
    payment_status = Column(String(40), nullable=False, default="unpaid")
...
class PaymentEvidence(Base):
    ...
    admin_validation_status = Column(String(40), nullable=False, default="evidence_received")
